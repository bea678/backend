...
export async function scrapeWinamaxTennis(browserParam) {
    ...
    try {
        ...
        while (!scrollFinalizado) {
            const extraccion = await page.evaluate(() => {
                ...
                for (const card of cards) {
                    ...
                    if (equipos.length >= 2) {
                        // Lógica adaptada para Tenis: suele haber 2 cuotas (1 y 2).
        ...
        console.log(`✅ [WINAMAX] Se han obtenido ${Object.keys(mapaResultados).length} partidos de Tenis.`);
        return mapaResultados;

    } catch (err) {
        console.error(`❌ [WINAMAX] Error durante el scraping de Tenis:`, err.message);
        return {};
    } finally {
        await page.close();
