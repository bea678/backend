...
/**
 * Parsea el texto crudo para Tenis
 */
function parsearPartidosBwin(textoCrudo) {
    ...
    for (let i = 0; i < lineas.length; i++) {
        // Detectar Torneo/Liga
        if (lineas[i].includes(' | ') || lineas[i].includes(' - TIERRA') || lineas[i].includes(' - PISTA')) {
            ligaActual = lineas[i];
            continue;
        }

        // Detectar marcador de tiempo o estado Live
        const esTiempo = /^(\d{2}:\d{2}|\(en \d+ min\)|Set \d+|LIVE|Finalizado.*)$/i.test(lineas[i]);
        ...
        if (esTiempo) {
            try {
                ...
                while (j < lineas.length && intentos < 30) {
                    ...
                    if (/^(\d{2}:\d{2}|\(en \d+ min\)|Set \d+|LIVE)$/i.test(lineaAnalizada)) break; 
                ...
                // Lógica Tenis: El "Ganador" (Money Line) es la primera columna.
                // Puppeteer lee por columnas, así que son los índices 0 y 1.
                if (cuotasLeidas.length >= 2) {
                    const cuotasMoneyLine = [cuotasLeidas[0], cuotasLeidas[1]];
...
// --- FUNCIÓN PRINCIPAL ---
export async function scrapeBwinTennis(browserParam) {    
    ...
    try {
        ...
        console.log('🌐 [BWIN-TENIS] Navegando a Tenis Hoy...');
        await page.goto('https://www.bwin.es/es/sports/tenis-5/hoy', { waitUntil: 'domcontentloaded', timeout: 80000 });

        console.log('⏳ [BWIN-TENIS] Esperando renderizado (9s)...');
        ...
        console.log('⏬ [BWIN-TENIS] Iniciando scroll...');
        ...
        console.log('📊 [BWIN-TENIS] Estructurando resultados...');
        todosLosPartidos.forEach((partido, i) => {
            try {
                // FILTRO: Descartar si es LIVE (En vivo)
                // En tenis Bwin pone "LIVE" o indica el set actual "1º Set", "2º Set"...
                if (/LIVE|Set \d+/i.test(partido.hora)) return; 
                ...
                if (key) {
                    mapaResultados[key] = {
                        eventId: `${i}_BWI_TEN_${partido.local.substring(0,3).toUpperCase()}`,
        ...
        //const outPath = path.join(process.cwd(), 'bwin_tenis.json');
        //fs.writeFileSync(outPath, JSON.stringify(mapaResultados, null, 4), 'utf8');
        
        console.log(`✅ [BWIN-TENIS] Éxito: ${Object.keys(mapaResultados).length} partidos guardados.`);
        return mapaResultados;

    } catch (error) {
        console.error("❌ [BWIN-TENIS] Error crítico:", error.message);
        return {};
    } finally {
        if (!browserParam) {
