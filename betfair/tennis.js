...
export async function scrapeBetfairTennis() {
    const url = 'https://www.betfair.es/sport/tennis';
    ...
    try {
        ...
        console.log('🎾 [BETFAIR TENIS] Navegando a la web...');
        await page.goto(url, { waitUntil: 'networkidle2', timeout: 60000 });

        console.log('🍪 [BETFAIR TENIS] Gestionando cookies...');
        try {
        ...
        console.log('📊 [BETFAIR TENIS] Extrayendo eventos...');
                ...
                for (const row of filas) {
                    ...
                    if (selecciones.length >= 2) {
                        ...
                        if (cuota1Elem && cuota2Elem) {
                            ...
                            const liga = row.querySelector('.event-link')?.getAttribute('data-competition') || "Tenis";
        ...
        console.log(`✅ [BETFAIR TENIS] Éxito: ${Object.keys(mapaResultados).length} partidos encontrados (Hoy).`);
        return mapaResultados;

    } catch (error) {
        console.error('❌ [BETFAIR TENIS] Error:', error.message);
        return {};
    } finally {
        await browser.close();
    }
