...
export async function scrapeBetfairBasketball() {
    const url = 'https://www.betfair.es/sport/basketball';
    ...
    try {
        ...
        console.log('🏀 [BETFAIR BALONCESTO] Navegando a la web...');
        await page.goto(url, { waitUntil: 'networkidle2', timeout: 60000 });

        console.log('🍪 [BETFAIR BALONCESTO] Gestionando cookies...');
        try {
        ...
        console.log('📊 [BETFAIR BALONCESTO] Extrayendo eventos...');
            ...
            for (const seccion of secciones) {
                ...
                for (const row of filas) {
                    ...
                    if (botones.length >= 2) {
                        ...
                        const liga = row.querySelector('.event-link')?.getAttribute('data-competition') || "Baloncesto";
        ...
        console.log(`✅ [BETFAIR BALONCESTO] Éxito: ${Object.keys(mapaResultados).length} partidos encontrados (Hoy).`);
        return mapaResultados;

    } catch (error) {
        console.error('❌ [BETFAIR BALONCESTO] Error:', error.message);
        return {};
    } finally {
        await browser.close();
    }
